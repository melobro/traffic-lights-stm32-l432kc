...
/* USER CODE BEGIN 0 */
  ...
  bool buttonclick () 
	{
	 button =  HAL_GPIO_ReadPin(GPIOB, BUTTON_Pin); 
	 bool off = !button && HAL_GPIO_ReadPin(GPIOB, BUTTON_Pin);
	 if(off){
		HAL_Delay(15);
	     if (HAL_GPIO_ReadPin(GPIOB, BUTTON_Pin)){ return true;}
		 }
		return false;
	}
